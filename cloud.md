...
>>>>>>> 19a2ff1f99836b70fe6a41c26eda19b2647c28c2
...
Next we will select our boot disk, this is the virtualized instance our machine will boot off of. For this tutorial we 
  will be picking Ubuntu 14.04 LTS 

Afterwards we will check the Allow HTTP traffic this will allow us to make requests to and from our backend database.

From this point on you will incure incremental charges for as log as the instance remains active. These charges are         negligable and will be convered as long as it doesnt exceed the credit we are given by google for creating our account. 
...
Once ssh finishes installing we wil linstall our backend 
...
```
next we will run our mongodb database in the bakground on port 80 
```
...
```
 exit 
```
 next we'll connect the frontend similiar to the backend
...
exit
```
_Remember to delete each of the two instances when you are done using them to avoid incuring payments_
